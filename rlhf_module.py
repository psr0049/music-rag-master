...
class RLHFTrainer:
    ...
    def get_adjusted_recommendations(self, query, base_recommendations, tracks_df):
        """
        Apply RLHF adjustments to base recommendations
        
        Args:
            query: User query
            base_recommendations: List of base recommended track IDs
            tracks_df: DataFrame containing track information
            
        Returns:
            Adjusted list of recommendations
        """
        if not hasattr(self, 'preference_adjustments'):
            return base_recommendations
        
        query_type = self._categorize_query(query)
        adjustment_factor = self.preference_adjustments.get(query_type, 1.0)
        
        # If adjustment factor suggests this query type performs poorly,
        # try to diversify recommendations
        if adjustment_factor < 1.0:
            # Add some diversity by including different types of tracks
            diverse_recs = self._add_diversity(base_recommendations, tracks_df)
            return diverse_recs
        
        return base_recommendations

    def _add_diversity(self, recommendations, tracks_df, diversity_factor=0.3):
        """
        Add diversity to recommendations when base approach isn't working well
        """
        if len(recommendations) == 0:
            return recommendations
        
        # Keep top recommendations but add some diverse ones
        keep_count = int(len(recommendations) * (1 - diversity_factor))
        diverse_count = len(recommendations) - keep_count
        
        # Get diverse tracks (different genres, different audio features)
        rec_tracks = tracks_df[tracks_df['track_id'].isin(recommendations[:keep_count])]
        
        if not rec_tracks.empty:
            # Find tracks with different characteristics
            avg_energy = rec_tracks['energy'].mean()
            avg_valence = rec_tracks['valence'].mean()
            
            # Look for tracks with different energy/valence
            diverse_candidates = tracks_df[
                (abs(tracks_df['energy'] - avg_energy) > 0.3) |
                (abs(tracks_df['valence'] - avg_valence) > 0.3)
            ]
            
            if len(diverse_candidates) > 0:
                diverse_tracks = diverse_candidates.sample(
                    n=min(diverse_count, len(diverse_candidates))
                )['track_id'].tolist()
                
                return recommendations[:keep_count] + diverse_tracks
        
        return recommendations

    def save_training_data(self, path="rlhf_training_data.json"):
        """
        ...
        print(f"💾 Saved {len(serializable_data)} training samples to {path}")

    def load_training_data(self, path="rlhf_training_data.json"):
        """
        ...
        try:
            ...
            for entry in serializable_data:
                internal_entry = {
                    ...
                    'recommendations': entry['recommendations'],
                    'feedback': entry['feedback'],
                    'timestamp': pd.Timestamp.fromisoformat(entry['timestamp'])
                }
                self.training_data.append(internal_entry)
            
            print(f"📂 Loaded {len(self.training_data)} training samples from {path}")
            
        except FileNotFoundError:
            print(f"⚠️ Training data file {path} not found")
        except Exception as e:
