    ...
    for filename in dataset_files:
        if os.path.exists(filename):
            try:
                ...
                df = loader.load_final_dataset(filename)
                st.success(f"✅ Successfully loaded dataset: {filename}")
                st.info(f"📊 Dataset contains {len(df):,} tracks from {df['artist_name'].nunique():,} artists")
                return df
...
def show_dataset_overview(df):
    ...
    with col2:
        st.metric("Unique Artists", f"{df['artist_name'].nunique():,}")
    with col3:
        st.metric("Unique Albums", f"{df['album_name'].nunique():,}")
    with col4:
        st.metric("Genres", f"{df['genre'].nunique()}")
    
    # Dataset sample
    st.subheader("📋 Dataset Sample")
    display_columns = ['track_name', 'artist_name', 'album_name', 'genre', 
    ...
    with col2:
        ...
        popular_tracks = df.nlargest(10, 'popularity')[['track_name', 'artist_name', 'popularity']]
...
def show_audio_features_analysis(df):
    ...
    fig = px.box(df, x='genre', y=selected_feature,
    ...
    with col1:
        ...
        fig = px.scatter(df, x=feature_x, y=feature_y, color='genre',
    ...
    with col2:
        ...
        if selected_features:
            avg_by_genre = df.groupby('genre')[selected_features].mean()
            fig = go.Figure()
...
def show_music_discovery(df):
    ...
    if len(filtered_df) > 0:
        # Display results
        display_cols = ['track_name', 'artist_name', 'album_name', 'genre',
                       'popularity', 'energy', 'danceability', 'valence']
        
        # Sort options
        sort_by = st.selectbox("Sort by:", ['popularity', 'energy', 'danceability', 'valence'], 
                              index=0)
...
def show_advanced_analytics(df):
    ...
    with tab1:
        ...
        with col2:
            ...
            cluster_genre = df_clustered.groupby(['cluster', 'genre']).size().unstack(fill_value=0)
    ...
    with tab3:
        ...
        genre_stats = df.groupby('genre')[audio_features].agg(['mean', 'std']).round(3)
        ...
        with col1:
            ...
            pop_by_genre = df.groupby('genre')['popularity'].agg(['mean', 'median', 'std']).round(2)
            st.dataframe(pop_by_genre)
        
        with col2:
            st.write("**Top Features Correlated with Popularity**")
            pop_corr = df[audio_features + ['popularity']].corr()['popularity'].drop('popularity')
...
def show_recommendations(df):
    ...
    if method == "🎨 Audio Features Based":
        ...
        if st.button("🎵 Get Recommendations"):
            ...
            popular_tracks = df[df['popularity'] >= min_popularity].copy()
            if len(popular_tracks) > 0:
                ...
                display_cols = ['track_name', 'artist_name', 'genre', 'popularity'] + audio_cols
        ...
        if st.button("🔍 Find Similar Artists"):
            ...
            artist_tracks = df[df['artist_name'] == selected_artist]
            if len(artist_tracks) > 0:
                ...
                selected_artist_genres = artist_tracks['genre'].value_counts()
                ...
                other_artists = df[df['artist_name'] != selected_artist]
                artist_similarities = []
                
                for artist in other_artists['artist_name'].unique():
                    artist_data = other_artists[other_artists['artist_name'] == artist]
                    if len(artist_data) > 0:
                        ...
                        if not other_profile.isna().any():
                            try:
                                ...
                                other_artist_genres = artist_data['genre'].value_counts()
                ...
                if len(similar_artists) > 0:
                    ...
                    similar_tracks = df[df['artist_name'] == top_similar].head(5)
                    
                    st.subheader(f"🎵 Sample tracks from {top_similar}")
                    display_cols = ['track_name', 'album_name', 'popularity']
        ...
        if selected_genres and st.button("🎵 Get Genre Recommendations"):
            ...
            genre_tracks = df[df['genre'].isin(selected_genres)]
            
            if len(genre_tracks) > 0:
                # Sort by popularity and get top tracks
                top_tracks = genre_tracks.nlargest(20, 'popularity')
                
                st.subheader(f"🎵 Top Tracks from {', '.join(selected_genres)}")
                display_cols = ['track_name', 'artist_name', 'genre', 'popularity', 'energy', 'danceability']
                ...
                audio_features = ['energy', 'danceability', 'valence', 'acousticness', 'instrumentalness']
                genre_profiles = genre_tracks.groupby('genre')[audio_features].mean()
                
                fig = go.Figure()
                for genre in genre_profiles.index:
                    fig.add_trace(go.Scatterpolar(
                        r=genre_profiles.loc[genre].values,
                ...
                st.warning("No tracks found for the selected genres.")
